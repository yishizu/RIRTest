...
if ghenv.Component.RunCount == 1:
    with Transaction(doc, ghenv.Component.NickName) as trans:
        ...
        with FilteredElementCollector(doc).OfClass(FamilyInstance).WhereElementIsNotElementType() as collector:
            ...
            els = collector.ToElements().Where(lambda x: x.get_Parameter(BuiltInParameter.ALL_MODEL_INSTANCE_COMMENTS).AsString() == ApplicationId).ToList[Element]()
            #update
            if len(els) >0:
                isCreated = True
            ...
            for e in els:
                i =0
                for id in I:
                    if id == e.get_Parameter(BuiltInParameter.ALL_MODEL_MARK).AsString():
                        
                        braceCurve1 = e.Location
                        braceCurve1.Curve = lines[i];
                    i= i+1
            braces = els
            ids  = braces.Select(lambda x: x.Id).ToList[ElementId]()
        trans.Commit()

if L is not None and isCreated is False:
    with Transaction(doc, ghenv.Component.NickName) as trans:
        trans.Start()
